 ...
 */
	class Vcode{
		...
		/**
		 * 用于输出验证码图片,也向服务器的session中保存了验证码,使用echo输出对象即可
		 */	
		function __toString(){
		...
		/*内部使用的私有方法,随机生成用户指定个数的字符串,去掉了容易混淆的字符oOLlZz和数字012 */
		...
		private function setDisturbColor(){
			...
			for ($i=0; $i < 10; $i++) { 
				$color = imagecolorallocate($this->image, rand(0,255), rand(0,255), rand(0,255));
				imagearc($this->image, rand(-10,$this->width), rand(-10,$this->height), rand(30,300), rand(30,300), 55, 44, $color);
			}
		}
